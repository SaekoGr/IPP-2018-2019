...
class Tests{
    ...
    public static function run_test($directory_path, $file, $parse_only, $int_only){
        ...
        if($parse_only){    # we test only parser
            exec('php7.2 ' . $GLOBALS['parse_script'] . ' < ' . $directory_path . '/' . $file . '.src > tmp_output_test', $exit_array, $exit_code); ### !!!!!!!!!!!! PHPHP VERSION
        ...
        elseif($int_only){  # we test only interpreter
            exec('python3.6 ' . $GLOBALS['int_script'] . ' --source=' . $directory_path . '/' . $file . '.src --input=' . $directory_path . '/' . $file . '.in > tmp_output_test', $exit_array, $exit_code); #### PYTHON VERSION
            if($exit_code == $expected_rc){
                if($exit_code == 0){    # also check the output
            ...
            exec('php7.2 ' . $GLOBALS['parse_script'] . ' < ' . $directory_path . '/' . $file . '.src > tmp_output_test_php', $exit_array, $exit_code); ### !!!!!!!!!!!! PHPHP VERSION
            exec('python3.6 ' . $GLOBALS['int_script'] . ' --source=tmp_output_test_php --input=' . $directory_path . '/'. $file . '.in > tmp_output_test', $exit_array, $exit_code); #### PYTHON VERSION
            if($exit_code == $expected_rc){
                if($exit_code == 0){ # also check the output
                    exec('diff tmp_output_test ' . $directory_path . "/" . $file . ".out > tmp_diff_output");
                    if(0 == filesize("tmp_diff_output")){
            ...
            exec("rm -f tmp_output_test tmp_output_test_php");
        }
    }
